...
toolbarApi = initToolbar({
  ...
  onToggleRightPanel: () => rightPanelApi?.toggle(),
});
refreshOrbitInfo();
...
rightPanelApi = initRightPanel({
  mount: rightMount,
  store,
  onOpenChange: (isOpen) => toolbarApi?.setRightPanelOpen(isOpen),
});
toolbarApi?.setRightPanelOpen(false);
const rightPanel = rightPanelApi;
