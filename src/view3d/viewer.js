...
export function createViewer({ container, initialLatitude, initialOrientationDeg = 0 }) {
  ...
  const initialWidth = clampSize(container.clientWidth);
  const initialHeight = clampSize(container.clientHeight);
  ...
  function resize(explicitWidth, explicitHeight) {
    const w = clampSize(explicitWidth ?? container.clientWidth);
    const h = clampSize(explicitHeight ?? container.clientHeight);
    renderer.setSize(w, h, false);
    camera.aspect = w / h;
    camera.updateProjectionMatrix();
    hasResolvedInitialSize = true;
  }

  let resizeObserver = null;
  let hasResolvedInitialSize = initialWidth > 1 || initialHeight > 1;

  function ensureInitialSize() {
    if (hasResolvedInitialSize) return;
    const { clientWidth, clientHeight } = container;
    if (clientWidth > 0 && clientHeight > 0) {
      hasResolvedInitialSize = true;
      resize(clientWidth, clientHeight);
    } else {
      requestAnimationFrame(ensureInitialSize);
    }
  }
